...
if($verify_result) {//验证成功
    ...
    if($_POST['trade_status'] == 'TRADE_FINISHED' ||$_POST['trade_status'] == 'TRADE_SUCCESS') {    //交易成功结束
		...
		// 取出充值人账户
		$result = mysql_query("select * FROM users_billing WHERE ID=".$out_trade_no);
		$row = mysql_fetch_array($result);
		$userID = $row['UserID'];
		
		mysql_query("UPDATE users_extension SET credit=credit+".$total_fee." WHERE UserID=".$userID);
		mysql_query("UPDATE users_billing SET paid=1, description='支付宝[".$buyer_email."]充值,支付宝交易号:".$trade_no."' WHERE ID=".$out_trade_no);	
